#!/usr/bin/env python3
"""Generate one image through the bundled Google Flow browser automation.

The WhatsApp agent calls this script after composing the final visual prompt.
The server keeps one authenticated Chromium session alive under Xpra. Agent
runs attach to that session over CDP, so Chromium is never closed between jobs.
"""
...
def output_path(raw: str | None) -> Path:
    path = (WORKSPACE / f"image-{int(time.time())}.png") if not raw else Path(raw).expanduser()
...
def main() -> int:
    ...
    job_id = f"wa-{int(time.time())}-{target.stem}"
    job_dir = JOBS / job_id
    images_dir = job_dir / "media" / "images"
    images_dir.mkdir(parents=True, exist_ok=True)
    prompt_file = images_dir / "prompts.plain.txt"
    prompt_file.write_text(args.prompt.strip() + "\n", encoding="utf-8")

    command = [
        sys.executable,
        str(ROOT / "flow_runner.py"),
        "--slug",
        job_id,
        "--project-dir",
        str(job_dir),
    ...
    for reference in args.references:
        ...
        if not reference_path.is_file():
            print(f"[image-gen] reference image not found: {reference_path}", file=sys.stderr)
            return 2
        command.extend(["--reference", str(reference_path)])

    print(f"[image-gen] generating {target.name}", flush=True)
    ...
    if not generated:
        print("[image-gen] Flow completed without an image", file=sys.stderr)
        return 1
    source = generated[-1]
    if source.suffix.lower() != target.suffix.lower():
        ...
        if converter:
            ...
            # Keep the original bytes if ImageMagick is unavailable; Flow's
            # magic bytes remain valid even when the requested suffix differs.
            shutil.copy2(source, target)
    else:
        shutil.copy2(source, target)
    print(f"IMAGE_GENERATED: {target}", flush=True)
    return 0
